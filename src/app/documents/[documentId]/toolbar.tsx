            ...
            <DropdownMenuContent className="p-1 flex flex-col gap-y-1">
                {lineHeights.map(({ label, value }) => (
                    <button className={cn(
                        ...
                        editor?.getAttributes("parapgraph").lineHeight === value && "bg-neutral-200/80"
...
const FontSizeButton = () => {
    ...
    return (
        <div className="flex items-center gap-x-0.5">
            ...
            {isEditing ? (
                <input type="text" value={inputValue} onChange={handleInputChange} onBlur={handleInputBlur} onKeyDown={handleKeyDown} className="h-7 w-10 text-sm text-center border border-neutral-400 rounded-sm bg-transparent focus:outline-none focus:ring-0" />
            ) : (
                <button onClick={() => {
                    setIsEditing(true);
...
const ListButton = () => {
    ...
    const lists = [
        { label: "Bullet List", isActive: () => editor?.isActive("bulletList"), icon: ListIcon, onClick: () => editor?.chain().focus().toggleBulletList().run() },
        { label: "Ordered List", isActive: () => editor?.isActive("orderedList"), icon: ListOrderedIcon, onClick: () => editor?.chain().focus().toggleOrderedList().run() },
    ];
        ...
        <DropdownMenu>
            ...
            <DropdownMenuContent className="p-1 flex flex-col gap-y-1">
                {headings.map(({ label, value, fontSize }) => (
                    ...
                    }} key={value} style={{ fontSize }} className={cn(
                        "flex items-center gap-x-2 px-2 py-1 rounded-sm hover:bg-neutral-200/80",
        ...
        <DropdownMenu>
            ...
            <DropdownMenuContent className="p-1 flex flex-col gap-y-1">
                {fonts.map(({ label, value }) => (
                    <button onClick={() => editor?.chain().focus().setFontFamily(value).run()} key={value} style={{ fontFamily: value }} className={cn(
                        "flex items-center gap-x-2 px-2 py-1 rounded-sm hover:bg-neutral-200/80",
